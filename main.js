...
async function aiModeratorCheck(text) {
    try {
        ...
        **Тобі заборонено аналізувати або блокувати текст на основі потенційної "провокації" чи "ідентифікації".**
        ...
        Твій вердикт (тільки одне слово: BLOCK або ALLOW. Пояснення):
        `;
...
async function aiTopicAnalyzer(text) {
    try {
        const prompt = `Проаналізуй текст: "${text}". Обери одну категорію зі списку: "Запитання ❓", "Подяка 🙏", "Технічне 💻", "Обговорення 🗣️", "Знайомство 🤝". Відповідай ТІЛЬКИ назвою категорії без зайвих слів.`;
